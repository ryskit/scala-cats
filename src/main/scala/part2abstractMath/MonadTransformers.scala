...
object MonadTransformers {
  ...
  val listOpTuples: OptionT[List, (Int, Char)] = for {
  ...
  val futureOfEither
    : EitherT[Future, String, Int] = EitherT.right(Future(45)) //wrap over Future(Right(45))

  /*
    TODO exercise
    We have a multi-machine cluster for your business which will receive a traffixc surge following a media appearance.
  ...
  // TODO 2
  def generateTrafficSpikeReport(s1: String,
                                 s2: String): AsyncResponse[String] =
    canWithstandSurge(s1, s2).transform {
      ...
      case Right(true) =>
        Right(
          s"Servers $s1 and $s2 can cope with the incoming spike NO PROBLEM!"
        )
    }

  def main(args: Array[String]): Unit = {
    println(listOpTuples.value)
