...
object UsingMonads {
  ...
  val aChangedLoading = loadingMonad.flatMap(anEither)(
    n => if (n % 2 == 0) Right(n + 1) else Left("Loading meaning of life.")
  )
  ...
  // TODO 1: the service layer API of a web app
  case class Connection(host: String, port: String)
  val config = Map("host" -> "location", "port" -> "4040")
  ...
  object OptionHttpService extends HttpService[Option] {
    ...
    override def issueRequest(connection: Connection,
