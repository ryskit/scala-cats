...
object FunctionState {
  ...
  val countAndSay: State[Int, String] = State(
    currentCount => (currentCount + 1, s"Counted $currentCount")
  )
  ...
  // pure FP with states
  val firstTransformation = State(
    (s: Int) => (s + 1, s"Added 1 to 10, obtained ${s + 1}")
  )
  val secondTransformation = State(
    (s: Int) => (s * 5, s"Multiplied with 5, obtained ${s * 5}")
  )
  ...
  def addToCart(item: String, price: Double): State[ShoppingCart, Double] =
    State { (cart: ShoppingCart) =>
      (ShoppingCart(item :: cart.items, price + cart.total), price + cart.total)
    }
  val danielsCart = for {
  ...
  // return a State data structure that, when run, will not change the state but will issue the value f(a)
  def inspect[A, B](f: A => B): State[A, B] =
    State { a: A =>
      (a, f(a))
    }
  // returns a State data structure that, when run, return the value of that state and makes no changes
